...
def sample_and_group(npoint, radius, nsample, xyz, points):
    """
    Input:
        npoint: number of points of the output, also the number of points to sample
        radius: 
        nsample: k, maximum number of points in each sample
    ...
    # eliminate the unsampled points for next call to this funcion
    ...
    # the following 2 lines are unused
    grouped_xyz = index_points(xyz, idx) # [B, npoint, nsample, C]
    grouped_xyz_norm = grouped_xyz - new_xyz.view(B, S, 1, C)

    # select out the embedded points with knn information
    grouped_points = index_points(points, idx) # [B, npoint, nsample, D]

    # subtract the center
    grouped_points_norm = grouped_points - new_points.view(B, S, 1, -1)

    # concatenate a center with its knn 
